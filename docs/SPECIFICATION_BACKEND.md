...
- **Parse** each peptide list file (`core/peptides.py`): CSV/TSV with two columns (sequence, quantity), read positionally; header names are not interpreted. Auto-detects the delimiter (tab if the first line contains one, else comma) and the header: the first row is a header only when its second cell is neither a number, nor a number once digit-grouping characters are removed (`1,000`, `1 000`), nor a missing-value token (`NA`, `N/A`, `#N/A`, `NULL`, `NONE`). Those cells mark a data row with an unusable quantity, which is a `PeptideParsingError` on line 1 rather than a row silently dropped as a column name; header names that merely contain digits (`Intensity 01`) are still headers. A data row with a non-empty value beyond the second column is rejected (trailing empty cells from spreadsheet exports are allowed), because it usually means a quantity with a thousands separator was split into two columns and reading only the second would silently record the wrong number. Normalizes sequences, in this order: (1) remove every bracketed group `[...]`, `(...)`, `{...}` including nested ones, whether it holds a mass delta (`PEPT[+79.966]IDE`) or a modification name (`C[Carbamidomethyl]`, `M(Oxidation (M))`, `(UniMod:4)`), together with a lowercase `n`/`c` terminal-modification marker attached to it (`n[42.0106]PEPTIDE`); markers are only recognised when the sequence contains an uppercase letter, because in an all-lowercase sequence a trailing `c` is a cysteine (`peptidec[57]` is `PEPTIDEC`); (2) strip flanking residues written as `K.PEPTIDE.R` or `-.PEPTIDE.K`; (3) uppercase and drop every remaining non-letter (`_PEPTIDE_`, `PEP*TIDE`). Named modifications must be removed as whole groups, not character by character: deleting only non-letters would turn `C[Carbamidomethyl]PEPTIDE` into `CCARBAMIDOMETHYLPEPTIDE`, which passes the alphabet check and silently never matches. Validates against amino acid alphabet. Rejects a row whose raw sequence repeats an earlier row verbatim, and merges rows whose different raw forms normalize to the same sequence by summing quantities.
...
3. **GO annotation (non-redundant union)**: Collect all direct GO terms from implied subjects, resolving each through `GODAG.resolve_term_id` (a secondary `alt_id` maps to the primary term that absorbed it; an obsolete term stays as itself so it can be reported as obsolete; an ID the ontology does not know is dropped). For each term, compute the transitive closure (ancestors via `is_a` and/or `part_of` edges) using `core/go.py: GODAG.get_closure`. The peptide's GO terms are the union of all closures: `GO(p) = ⋃_{s∈S(p)} closure(go_terms(s))`.
...
SQLite database at `<DATA_DIR>/metagomics2.db` with three tables. It runs in write-ahead-logging mode (set once at initialisation; `metagomics2.db-wal` and `metagomics2.db-shm` sidecar files appear beside it while a connection is open and SQLite removes them when the last connection closes) and every connection waits up to 30 s for a lock, because the server and the worker write it from separate processes:
